<%-- 
    Document   : moddirector
    Created on : 18-feb-2020, 12:15:03
        ...
        <%
             if(session.getAttribute("rol_user").toString().matches("2")){
              ...
              String host = InetAddress.getLocalHost().toString();
               int res = dir.updatedirector(session.getAttribute("id_director").toString(), request.getParameter("nombre"), request.getParameter("ap_paterno"), request.getParameter("ap_materno"), request.getParameter("telefono"), request.getParameter("email"), request.getParameter("rfc"),request.getParameter("status"),host);
                             
                if(res == 0){
                     response.sendRedirect("directores.htm?msj=Error&e=Fallo al Actualizar Director");
                }
                else if(res == 2){
                     response.sendRedirect("directores.htm?msj=Exists");
                }
                else if(res == 1){
                    session.removeAttribute("id_director");
                    response.sendRedirect("directores.htm?msj=UpSuccess");
                }
                }
                catch(Exception e){
